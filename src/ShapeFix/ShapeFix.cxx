                                         ...
                                         const Handle(ShapeExtend_BasicMsgRegistrator)& theMsgReg)
{
  ...
  Message_ProgressScope aPSForSameParam(theProgress, "Fixing same parameter problem", 0, 2, 1);

  {
    // Start progress scope (no need to check if progress exists -- it is safe)
    Message_ProgressScope aPS(&aPSForSameParam, "Fixing edge", 0, aNbEdges, 1);
  ...
  aPSForSameParam.Next();

  {
    // Start progress scope (no need to check if progress exists -- it is safe)
    Message_ProgressScope aPS(&aPSForSameParam, "Update tolerances", 0, aNbFaces, 1);
