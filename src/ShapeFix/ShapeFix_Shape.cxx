...
Standard_Boolean ShapeFix_Shape::Perform(Message_ProgressScope* theProgress) 
{
  ...
  Message_ProgressScope aPS(theProgress, "Fixing stage", 0, 2, 1);
  ...
  case TopAbs_COMPSOLID: {
    ...
    Message_ProgressScope aPSSubShape(&aPS, "Fixing sub-shape", 0, aShapesNb, 1);
    for ( TopoDS_Iterator anIter(S); anIter.More() && aPSSubShape.More(); anIter.Next(), aPSSubShape.Next() )
    {
      myShape = anIter.Value();
      if ( Perform(&aPSSubShape) )
        status = Standard_True;
    }
    if ( !aPSSubShape.More() )
  ...
  {
    SameParameter(myResult, Standard_False, &aPS);
    if (!aPS.More())
      return Standard_False; // aborted execution
  }
