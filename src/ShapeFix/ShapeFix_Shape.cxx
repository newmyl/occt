...
Standard_Boolean ShapeFix_Shape::Perform(Message_ProgressScope* theProgress) 
{
  ...
  Message_ProgressScope aPSentry(theProgress, "Fixing stage", 0, 2, 1);
  ...
  case TopAbs_SOLID: {  
    ...
    if ( myFixSolid->Perform(&aPSentry) )
      status = Standard_True;
  ...
  {
    SameParameter(myResult, Standard_False, &aPSentry);
    if (!aPSentry.More())
      return Standard_False; // aborted execution
  }
