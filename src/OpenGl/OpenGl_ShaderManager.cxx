                                               ...
                                               const Standard_Boolean   isGetColorVar)
{
  ...
  else if (((theBits & OpenGl_PO_AdvShadingMods) >> 13) == OpenGl_PO_SolidWFMode)
  {
    theSrcFragMain += TCollection_AsciiString()
      + EOL"  float aDistance = min (min (EdgeDistance[0], EdgeDistance[1]), EdgeDistance[2]);"
        EOL"  float aMixVal   = smoothstep (0.0, occLineWidth + 0.5, aDistance);"
        EOL"  vec3 anEdgeColor;"
        EOL"  if (occIsColoringEdge)"
        EOL"    anEdgeColor = occWireframeColor;"
        EOL"  else"
        EOL"    anEdgeColor = " + ((theBits & OpenGl_PO_VertColor) != 0 ? "VertColor.rgb" : "occColor.rgb") + " - 0.07f;"
        EOL"  occSetFragColor (mix (vec4 (anEdgeColor.x, anEdgeColor.y, anEdgeColor.z, 1.0), " + (isGetColorVar ? "getColor()" : "aColor") + ", aMixVal));";
  }
  else
  {
    theSrcFragMain += TCollection_AsciiString()
      + EOL"  occSetFragColor (" + (isGetColorVar ? "getColor()" : "aColor") + ");";
  }
}
...
// purpose  :
// =======================================================================
TCollection_AsciiString OpenGl_ShaderManager::prepareGeomMainSrc (const OpenGl_ShaderObject::ShaderVariableList& theStageInOuts)
{
  TCollection_AsciiString aSrcMainGeom =
    ...
    EOL"  if (occIsQuadMode) aQuadModeHeightC = occLineWidth + 1.0;";
  for (Standard_Integer aTriVertIter = 0; aTriVertIter < 3; ++aTriVertIter)
  {
    ...
    {
      const Standard_Boolean isGeomShaderFormat = (aVarListIter.Value().Stages == (Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT | Graphic3d_TOS_GEOMETRY));
      if (!isGeomShaderFormat)
      {
        continue;
      }
      TCollection_AsciiString aVarName = aVarListIter.Value().Name.Token (" ", 2);
                                                               ...
                                                               const Standard_Integer        theBits)
{
  ...
  TCollection_AsciiString aSrcFrag, aSrcFragExtraMain, aSrcFragMainGetColor;
  ...
  else
  {
    ...
    {
      aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 TexCoord", toUseInAllStages));
    }
  ...
  {
    aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 VertColor", toUseInAllStages));
  ...
  {
    aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 PositionWorld", toUseInAllStages));
    aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 Position",      toUseInAllStages));
    aSrcVertExtraMain +=
  ...
  if ((theBits & OpenGl_PO_StippleLine) != 0)
  {
    ...
    if (hasGlslBitOps)
    {
      ...
      aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec2 ScreenSpaceCoord", toUseInAllStages));
      aSrcVertEndMain =
  ...
  if (toUseGeomShader)
  {
    aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec3 EdgeDistance",      Graphic3d_TOS_GEOMETRY | Graphic3d_TOS_FRAGMENT));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("float occLineWidth",     Graphic3d_TOS_FRAGMENT));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("vec3 occWireframeColor", Graphic3d_TOS_FRAGMENT));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("bool occIsColoringEdge", Graphic3d_TOS_FRAGMENT));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("vec4 occViewport",       Graphic3d_TOS_GEOMETRY));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("bool occIsQuadMode",     Graphic3d_TOS_GEOMETRY));
    aUniforms.Append    (OpenGl_ShaderObject::ShaderVariable ("float occLineWidth",     Graphic3d_TOS_GEOMETRY));
    aSrcGeom = prepareGeomMainSrc (aStageInOuts);
  }
  ...
  if (toUseGeomShader)
  {
    aProgramSrc->AttachShader (OpenGl_ShaderObject::CreateFromSource (aSrcVert, Graphic3d_TOS_VERTEX,   aUniforms, aStageInOuts, "geomIn", "geomOut", 3));
    aProgramSrc->AttachShader (OpenGl_ShaderObject::CreateFromSource (aSrcGeom, Graphic3d_TOS_GEOMETRY, aUniforms, aStageInOuts, "geomIn", "geomOut", 3));
    aProgramSrc->AttachShader (OpenGl_ShaderObject::CreateFromSource (aSrcFrag, Graphic3d_TOS_FRAGMENT, aUniforms, aStageInOuts, "geomIn", "geomOut", 3));
  }
  else
  {
    aProgramSrc->AttachShader (OpenGl_ShaderObject::CreateFromSource (aSrcVert, Graphic3d_TOS_VERTEX, aUniforms, aStageInOuts));
                                                                 ...
                                                                 const Standard_Integer        theBits)
{
  ...
  aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 FrontColor", toUseInAllStages));
  aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 BackColor",  toUseInAllStages));
  aSrcVertExtraMain +=
                                                               ...
                                                               const Standard_Boolean        theIsFlatNormal)
{
  ...
  {
    aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec3 Normal", toUseInAllStages));
    aSrcVertExtraFunc += THE_FUNC_transformNormal;
  ...
  aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 PositionWorld", toUseInAllStages));
  aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec4 Position",      toUseInAllStages));
  aStageInOuts.Append (OpenGl_ShaderObject::ShaderVariable ("vec3 View",          toUseInAllStages));
  prepareFragExtrSrc (aSrcFragExtraMain, theBits, true);
