                         ...
                         const char**      theArgs)
{
  ...
  if (aFillAreaAspect->Aspect()->InteriorStyle() == Aspect_IS_SOLID && !aFillAreaAspect->Aspect()->IsAdvancedShading())
  {
  ...
  for (Standard_Integer anIt = 2; anIt < theArgNum; ++anIt)
  {
    ...
    if (aParam.Value (1) == '-' && !aParam.IsRealValue())
    {
      if (aParam.IsEqual ("-type"))
      {
        ...
        if (aTypeEnum != Aspect_TOL_SOLID && aFillAreaAspect->Aspect()->InteriorStyle() == Aspect_IS_SOLID_WIREFRAME && !aFillAreaAspect->Aspect()->IsAdvancedShading())
        {
