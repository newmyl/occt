...
// Amelioration of the filling aglorithm
// 29-09-97 ; jct; correction of calculation of of double  points (tolerance 2d and not 3d)
// 16-10-97 ; jct; use Raise instead of cout in debug (otherwise crash in k1fab)
// 03-11-97 ; jct; more references to BRepAdaptor and  BRepTools
...
#include <stdio.h>
// for G2 checking 
// for intersections between curves
// projection
#ifdef DRAW
...
{ Standard_Integer NTCurve=TabCurve->Length();// Number of linear constraints
  ...
  // Filling fields passing from the old constructor to the new
  for(i=1;i<=NTCurve;i++) 
...
//------------------------------------------------------------------
// Constructor with surface of init and degree of resolution of plate
//------------------------------------------------------------------
GeomPlate_BuildPlateSurface::GeomPlate_BuildPlateSurface ( 
...
{   if (myNbIter<1)
    throw Standard_ConstructionError("GeomPlate :  Number of iteration must be >= 1");
 if (myDegree<2) 
     throw Standard_ConstructionError("GeomPlate : the degree resolution must be above 2");
   myLinCont = new GeomPlate_HSequenceOfCurveConstraint;
   myPntCont = new GeomPlate_HSequenceOfPointConstraint;
...
//---------------------------------------------------------
// Constructor with degree of resolution of plate
//---------------------------------------------------------
GeomPlate_BuildPlateSurface::GeomPlate_BuildPlateSurface (
                             const Standard_Integer Degree,
			     const Standard_Integer NbPtsOnCur,
...
//---------------------------------------------------------
// Function : Perform
// Calculated the surface filled with loaded constraints
//---------------------------------------------------------
...
{ 
  ...
  // Variable  NTPoint can be removed
  ...
  if (!mySurfInitIsGive)
   ...
   if (NTLinCont>=2)
	{ // Table of transformations to preserver the initial order see TrierTab
	...
	{ //====================================================================
	  ...
	  if (!aProgress.IsNull() && aProgress->UserBreak())
	  {
	    return;
          ...
          {
#ifdef OCCT_DEBUG
            cout << "WARNING : GeomPlate : abort calculation of Plate." << endl;
#endif
            return;
          }
...
void GeomPlate_BuildPlateSurface::ComputeSurfInit(const Handle(Message_ProgressIndicator) & aProgress)
{
  ...
  if (NTLinCont != 0)
    TrierTab( myInitOrder ); // Reorder the tableau of transformations 
  ...
  if (nopt != 3)
	...
	{//    throw Standard_Failure("Curves are not joined");
#ifdef OCCT_DEBUG	    
	  cout<<"WARNING : Curve non-adjacent to "<<myTol3d<<" pres"<<endl;
#endif	  
	  nopt = 1;
	}
		  ...
		  if (P1.Distance( P2 ) < myTol3d)
			...
			{ gp_Vec v11,v12,v13,v14,v15,v16,v21,v22,v23,v24,v25,v26;
			  ...
			  if ((Abs(v16*v15-v16*v25)>(myTol3d/1000))
			    ...
			    // constraint G1 corresponding 
			    // to 3D tolerance of 0.01
			    { Standard_Real coin;
			  ...
			  (myLinCont->Value(i)->Order()==1 && myLinCont->Value(j)->Order()==0))
			{
			  ...
			  if (Angle > myTolAng/10.) //????????? //if (Abs( scal ) > myTol3d/100)
			    { // Non-compatible ==> one removes zone in 
			    // constraint G0 and G1 corresponding 
			    // to 3D tolerance of 0.01
			      Standard_Real coin;
			      Standard_Real Tol= 100 * myTol3d;
			      Standard_Real A1;
...
#ifdef OCCT_DEBUG
		    cout<<"Attention: Two points 3d have the same projection dist="
		      <<Dist<<endl;
#endif	
#ifdef DRAW
		    if (Affich > 1)
...
// Field myPlateCont contiendera les parametre des points envoye a plate
// it excludes duplicate points and imcompatible zones.
// The first part corresponds to the verification of compatibility
// and to the removal of duplicate points.
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::
...
//    ACR = (!HC2d.IsNull() || !C2d.IsNull());
    ...
    ACR =  (!C2d.IsNull());
    if (ACR) {
      // One constructs a law close to curvilinear abscissa
...
/*        if (!HC2d.IsNull()) {
    ...
    { 
      ...
      myParCont->ChangeValue(i).Append(Inter);// one adds the point
      if (NbPtInter!=0) 
      { 
        for(Standard_Integer l=1;l<=NbPtInter;l+=2) 
        {
          // one finds if the point Inter is in the interval 
          ...
          { 
            l=NbPtInter+2; 
            // to leave the loop without storing the point	 
          }
          else
          {
            if (l+1>=NbPtInter) {
      ...
      {
        if (NbPtG1G1!=0) // does there exist an incompatible interval 
        {
          for(Standard_Integer k=1;k<=NbPtG1G1;k+=2)
          {
            if ((Inter>PntG1G1->Value(i).Value(k))
        ...
        else
        {
          if (  ( (!mySurfInitIsGive)
             ...
             || ( (j>1) &&(j<NbPnt_i))) //one removes the extremes
            myPlateCont->ChangeValue(i).Append(Inter);// one adds the point
        }
      }
    }
  } 
...
// Function : LoadCurve
//---------------------------------------------------------
// Starting from table myParCont load all points noted in plate
//---------------------------------------------------------
...
VerifSurface(const Standard_Integer NbBoucle)
{
  ...
  for (Standard_Integer i=1;i<=NTLinCont;i++) {
	...
	{ if (tdist->Value(j)>myG0Error)
	  ...
	  // fond the maximum variation of error and calculate the average
	  if (diffDist>0) {
