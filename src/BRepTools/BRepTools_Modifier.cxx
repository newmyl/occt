                                 ...
                                 Message_ProgressScope* theProgr)
{
  ...
  Message_ProgressScope aPS(theProgr, "Converting Shape", 0, 2, 1);
  ...
  Rebuild(myShape, M, aNewGeom, &aPS);
...
//	result.Orientation(S.Orientation());
...

  {
    ...
    Message_ProgressScope aPS(theProgr, "Converting SubShapes", 0, aShapeCount, 1);
    //
    for (it.Initialize(S, Standard_False); it.More() && aPS.More(); it.Next(), aPS.Next()) {
      // always call Rebuild
      Standard_Boolean isSubNewGeom = Standard_False;
      Standard_Boolean subrebuilt = Rebuild(it.Value(), M, isSubNewGeom, &aPS);
