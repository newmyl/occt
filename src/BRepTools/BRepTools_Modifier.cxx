                                 ...
                                 Message_ProgressScope* aProgress)
{
  ...
  Message_ProgressScope aPSentry(aProgress, "Converting Shape", 0, 2, 1);
  ...
  Rebuild(myShape, M, aNewGeom, &aPSentry);
...
//	result.Orientation(S.Orientation());
...

  {
    ...
    Message_ProgressScope aPSentry(aProgress, "Converting SubShapes", 0, aShapeCount, 1);
    //
    for (it.Initialize(S, Standard_False); it.More() && aPSentry.More(); it.Next(), aPSentry.Next()) {
      ...
      Standard_Boolean subrebuilt = Rebuild(it.Value(), M, isSubNewGeom, &aPSentry);
