...
public:
  ...
  Standard_EXPORT const Standard_Boolean AddApplication(const Handle(CDF_Application)& theApp, const Standard_Integer theID);

  //! returns true if theApp is removed from the session
  Standard_EXPORT const Standard_Boolean RemoveApplication(const Standard_Integer theID);

  //! returns true if theApp is removed from the session
  Standard_EXPORT const Standard_Boolean FindApplication(const Standard_Integer theID, Handle(CDF_Application)& theApp) const;
