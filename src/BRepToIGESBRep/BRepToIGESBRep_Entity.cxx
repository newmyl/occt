                                                              ...
                                                              Message_ProgressScope* theProgr)
{
  ...
  for (Ex.Init(start, TopAbs_FACE); Ex.More(); Ex.Next())
    nbf++;
  Message_ProgressScope aPS(theProgr, NULL, 0, nbf);
                                                                       ...
                                                                       Message_ProgressScope* theProgr)
{
  ...
  for (Ex.Init(start,TopAbs_SHELL); Ex.More() && aPS.More(); Ex.Next(), aPS.Next()) {
    TopoDS_Shell S = TopoDS::Shell(Ex.Current());
    if (S.IsNull()) {
      AddWarning(start," a Shell is a null entity");
    }
    else {
      IShell = TransferShell(S, &aPS);
      if (!IShell.IsNull()) { 
	Seq->Append(IShell);
                                                                      ...
                                                                      Message_ProgressScope* theProgr)
{
  ...
  for (Ex.Init(start, TopAbs_SOLID); Ex.More(); Ex.Next())
    nbs++;
  Message_ProgressScope aPS(theProgr, NULL, 0, nbs);
  for (Ex.Init(start,TopAbs_SOLID); Ex.More() && aPS.More(); Ex.Next(), aPS.More()) {
    TopoDS_Solid S = TopoDS::Solid(Ex.Current());
    if (S.IsNull()) {
      AddWarning(start," an Solid is a null entity");
    }
    else {
      ISolid = TransferSolid(S, &aPS);
                                                                     ...
                                                                     Message_ProgressScope* theProgr)
{
  ...
  for (Ex.Init(start, TopAbs_VERTEX, TopAbs_EDGE); Ex.More(); Ex.Next())
    nbshapes++;
  Message_ProgressScope aPS(theProgr, NULL, 0, nbshapes);

  // take all Solids
  for (Ex.Init(start, TopAbs_SOLID); Ex.More() && aPS.More(); Ex.Next(), aPS.Next()) {
    TopoDS_Solid S = TopoDS::Solid(Ex.Current());
    if (S.IsNull()) {
      AddWarning(start," a Solid is a null entity");
    }
    else {
      IShape = TransferSolid(S, &aPS);
      if (!IShape.IsNull()) Seq->Append(IShape);
    }
  }

  // take all isolated Shells 
  for (Ex.Init(start, TopAbs_SHELL, TopAbs_SOLID); Ex.More() && aPS.More(); Ex.Next(), aPS.Next()) {
    TopoDS_Shell S = TopoDS::Shell(Ex.Current());
    if (S.IsNull()) {
      AddWarning(start," a Shell is a null entity");
    }
    else {
      IShape = TransferShell(S, &aPS);
      if (!IShape.IsNull()) Seq->Append(IShape);
    }
  }
