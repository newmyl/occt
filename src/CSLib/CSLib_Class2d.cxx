                 ...
                 CSLib_Class2d& theObj)
{
  ...
  {
    theObj.MyPnts2dX.Nullify();
    theObj.MyPnts2dY.Nullify();
    theObj.N = 0;
  }
  //
  else
  {
    ...
    //
    aPrc = 1.e-10;
    theObj.N = TP2d.Length();
    theObj.Tolu = aTolu;
    theObj.Tolv = aTolv;
    theObj.MyPnts2dX = new TColStd_Array1OfReal(0, theObj.N);
    theObj.MyPnts2dY = new TColStd_Array1OfReal(0, theObj.N);
    du = umax - umin;
    dv = vmax - vmin;
    //
    iLower = TP2d.Lower();
    for (i = 0; i<theObj.N; ++i)
    {
      const gp_Pnt2d& aP2D = TP2d(i + iLower);
      theObj.MyPnts2dX->ChangeValue(i) = Transform2d(aP2D.X(), umin, du);
      theObj.MyPnts2dY->ChangeValue(i) = Transform2d(aP2D.Y(), vmin, dv);
    }
    theObj.MyPnts2dX->ChangeLast() = theObj.MyPnts2dX->First();
    theObj.MyPnts2dY->ChangeLast() = theObj.MyPnts2dY->First();
    //
    if (du>aPrc)
    {
      theObj.Tolu /= du;
